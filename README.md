# Hi, I'm Tanmay 👋

Java Backend Developer focused on building scalable backend applications with Spring Boot.

## About Me

- Building backend projects using Java and Spring Boot
- Learning REST APIs, Validation, Exception Handling, MapStruct, and preparing for Spring Data JPA & Hibernate
- Practicing Data Structures & Algorithms for technical interviews
- Interested in Backend Development, Software Engineering, and Clean Architecture
- Open to internships and backend development opportunities

## Tech Stack

![Java](https://img.shields.io/badge/Java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-%236DB33F.svg?style=for-the-badge&logo=springboot&logoColor=white)
![Apache Maven](https://img.shields.io/badge/Apache%20Maven-C71A36?style=for-the-badge&logo=ApacheMaven&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)

## Featured Projects

### Spring Learning Lab
A Spring Boot learning project covering:
- REST APIs
- DTOs
- Validation
- Global Exception Handling
- MapStruct
- Layered Architecture

### Task Manager
Desktop application built with Java and JavaFX featuring:
- Clean architecture
- File persistence
- Task analytics
- Search and filtering
- Modular project structure

### DSA Practice
Collection of Java solutions focused on:
- Arrays
- Strings
- Recursion
- Searching
- Sorting
- Interview-oriented problem solving

## Currently Learning

- Database Design
- SQL
- Spring Data JPA
- Hibernate
- Advanced Spring Boot

## GitHub Activity

![](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Tanmay1880&theme=github_dark)

![GitHub Streak](https://streak-stats.demolab.com?user=Tanmay1880&theme=github-dark&hide_border=true)

![](https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Tanmay1880&theme=github_dark)


## Goals

- Build production-ready backend applications
- Contribute to open-source projects
- Strengthen DSA and problem-solving skills
- Secure a Java Backend Developer internship
